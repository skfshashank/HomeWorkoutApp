...
export class DashboardView {
  ...
  async render() {
    ...
    this.el.innerHTML = `
      ...
      <section class="card"><div class="flex flex-between gap-12 mb-16"><div><h2>${this.t('today_workout', "Today's Workout")}</h2><p class="text-sm text-muted">${this.t('today_workout_subtitle', 'Generated from your goal, level, streak, and plan library.')}</p></div><span class="badge badge-${user.level}">${user.level}</span></div><div class="workout-card"><div class="flex flex-between gap-12"><div><strong>${this.translatePlanName(this.todaysPlan)}</strong><p class="text-sm text-muted">${this.todaysPlan.description}</p></div><div class="flex flex-col gap-8"><span class="chip">${this.todaysPlan.estimatedMinutes} ${this.t('minutes', 'min')}</span><span class="chip">${this.todaysPlan.estimatedCalories} ${this.t('calories', 'kcal')}</span></div></div><div class="flex flex-wrap gap-8">${this.todaysPlan.preview.map((item) => `<span class="chip">${item}</span>`).join('')}</div><div class="flex gap-12 mt-16"><button class="btn btn-primary" data-action="start-today">${this.todaysPlan.isRestDay ? this.t('start_recovery_flow', 'Start recovery flow') : this.t('start_workout', 'Start workout')}</button><button class="btn btn-secondary" data-action="browse-workouts">${this.t('browse_plans', 'Browse plans')}</button></div></div></section>
