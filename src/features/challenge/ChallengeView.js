    ...
    this.el.innerHTML = `
      ...
      <section class="card">
        ...
        <div class="mb-16">${selected.exercises.map((item) => {
          ...
          return `<div class="history-item"><div class="history-info"><h4>${exercise?.emoji || '💪'} ${exercise?.name || item.exerciseId}</h4><p>${item.sets} ${this.t('sets', 'Sets').toLowerCase()}${item.sets > 1 ? '' : ''} • ${exercise?.type === 'time' ? `${exercise.getTarget(user.level)} sec` : `${exercise?.getTarget(user.level)} ${this.t('reps', 'Reps').toLowerCase()}`}</p></div></div>`;
        }).join('')}</div>
